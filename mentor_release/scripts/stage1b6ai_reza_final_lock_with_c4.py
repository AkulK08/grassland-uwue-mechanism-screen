...
ROOT = Path(".")
OUT = Path("results/stage1b6ai_reza_final_lock_with_c4")
TAB = OUT / "tables"
TXT = OUT / "text"
FIG = OUT / "figures"
...
# -----------------------------------------------------------------------------
screen_audit_path = Path("results/stage1b6ah_reza_protocol_analysis_lock/tables/Table_PRODUCT03ab_product_screening_audit.csv")
screening_answer = {
    "agreement_filter_keyword_hits": np.nan,
    "quality_filter_keyword_hits": np.nan,
    "answer_for_reza": "unknown; prior audit file not found",
}
if screen_audit_path.exists():
    ...
    if audit is not None and len(audit):
        ...
        screening_answer = {
            ...
            "answer_for_reza": "Audit file existed but was empty."
        }

pd.DataFrame([screening_answer]).to_csv(TAB / "Table_PRODUCT03al_product_screening_answer_for_reza.csv", index=False)

# -----------------------------------------------------------------------------
# B. Clean tower validation table from prior deliverable
# -----------------------------------------------------------------------------
prior_tower = Path("results/stage1b6ah_reza_protocol_analysis_lock/tables/Table_PRODUCT03ac_tower_validation_deliverable.csv")
...
if len(clean_tower):
    clean_tower["passes_reza_quality_fields"] = (
...
clean_tower.to_csv(TAB / "Table_PRODUCT03an_reza_clean_tower_validation_table.csv", index=False)

if len(clean_tower):
    et_rank = (
        clean_tower[clean_tower["passes_reza_quality_fields"]]
...
et_rank.to_csv(TAB / "Table_PRODUCT03ao_reza_clean_et_product_ranking.csv", index=False)
...
final_decision = {
    ...
    "stage": "1B.6AI_Reza_final_lock_with_C4",
    ...
    "what_is_still_missing_for_reza": [
...
(TAB / "STAGE1B6AI_REZA_FINAL_LOCK_DECISION.json").write_text(json.dumps(final_decision, indent=2), encoding="utf-8")
...
# Report.
report = []
report.append("# Stage 1B.6AI Reza final lock with C4")
report.append("")
...
report.append("")
report.append(screening_answer["answer_for_reza"])
report.append("")
report.append("```text")
...
report.append("")
for p in [
    "Table_PRODUCT03al_product_screening_answer_for_reza.csv",
    "Table_PRODUCT03am_tower_flux_quality_closure_gapfill.csv",
    "Table_PRODUCT03an_reza_clean_tower_validation_table.csv",
    "Table_PRODUCT03ao_reza_clean_et_product_ranking.csv",
    ...
    "STAGE1B6AI_REZA_FINAL_LOCK_DECISION.json",
]:
...
report_text = "\n".join(report)
(TXT / "STAGE1B6AI_REZA_FINAL_LOCK_REPORT.md").write_text(report_text, encoding="utf-8")

print(report_text)
print("")
print("WROTE", TXT / "STAGE1B6AI_REZA_FINAL_LOCK_REPORT.md")
print("WROTE", TAB / "STAGE1B6AI_REZA_FINAL_LOCK_DECISION.json")
