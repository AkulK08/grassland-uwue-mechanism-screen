...
OUT = Path("results/stage1b6ad_regional_hotspot_trait_atlas_NAMEDPERM500")
TAB = OUT / "tables"
TXT = OUT / "text"
FIG = OUT / "figures"
...
for k in [6, 8]:
    ...
    vc = coords[valid_idx]
    for center_pos, orig_i in enumerate(valid_idx[::max(1, len(valid_idx)//20)]):
...
for c in candidates:
    ...
    for outcome in OUTCOMES[:3]:
        for control_name, controls_raw in {k:v for k,v in CONTROL_SETS.items() if k in ['none','parsimonious_aridity_temp_lai_soil']}.items():
