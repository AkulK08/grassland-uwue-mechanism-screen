...
ROOT = Path(".").resolve()
OUT = Path("results/stage1b6ah_reza_protocol_analysis_lock")
TAB = OUT / "tables"
TXT = OUT / "text"
FIG = OUT / "figures"
...
final_decision = {
    "generated": now(),
    "stage": "1B.6AH_Reza_protocol_analysis_lock",
...
(TAB / "STAGE1B6AH_REZA_PROTOCOL_ANALYSIS_LOCK_DECISION.json").write_text(json.dumps(final_decision, indent=2), encoding="utf-8")

report = []
report.append("# Stage 1B.6AH Reza protocol analysis lock")
report.append("")
...
report.append("")
report.append("A dated pre-registration file was written to `results/stage1b6ah_reza_protocol_analysis_lock/text/PREREGISTERED_C3C4_ANALYSIS_PLAN.md`.")
report.append("")
...
report_text = "\n".join(report)
(TXT / "STAGE1B6AH_REZA_PROTOCOL_ANALYSIS_LOCK_REPORT.md").write_text(report_text, encoding="utf-8")
...
print("WROTE", TAB / "STAGE1B6AH_REZA_PROTOCOL_ANALYSIS_LOCK_DECISION.json")
print("WROTE", TXT / "PREREGISTERED_C3C4_ANALYSIS_PLAN.md")
print("WROTE", TXT / "STAGE1B6AH_REZA_PROTOCOL_ANALYSIS_LOCK_REPORT.md")
