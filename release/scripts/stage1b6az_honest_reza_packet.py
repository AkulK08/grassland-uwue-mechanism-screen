...
ROOT = Path.cwd()
IN = ROOT / "results/stage1b6ay_final_reza_audit"
TAB_IN = IN / "tables"

OUT = ROOT / "results/stage1b6az_honest_reza_packet"
TAB = OUT / "tables"
TXT = OUT / "text"
TAB.mkdir(parents=True, exist_ok=True)
TXT.mkdir(parents=True, exist_ok=True)

DECISION_PATH = TAB_IN / "STAGE1B6AY_FINAL_REZA_AUDIT_DECISION.json"
...
def main():
    ...
    concern_rows = [
        {
            "reza_concern": "Full climate/environment controls",
            "status": "NOT_SOLVED",
        ...
        {
            "reza_concern": "C4 cropland leakage in satellite sample",
        ...
        {
            "reza_concern": "Tower land-cover filtering, especially US-Ne1/2/3",
    ...
    concern.to_csv(TAB / "Table_PRODUCT03ft_reza_concern_status_table.csv", index=False)

    final_decision = {
        "generated": datetime.now().isoformat(timespec="seconds"),
        "stage": "1B.6AZ_honest_reza_packet",
        "bottom_line": "Do not claim that all Reza concerns are solved. The clean audit says the C4 effect is sensitivity-dependent and fails the full climate/environment control screen.",
        ...
        "recommendation_for_meeting": "Ask Reza whether baseline VPD should be treated as a confounder or as part of the stress pathway. If confounder, the clean C4 mechanism should be rejected/narrowed. If pathway/mediator, the paper can be reframed as a C4-VPD climate-syndrome result.",
        "all_reza_concerns_solved": False,
    }
    (TAB / "STAGE1B6AZ_HONEST_REZA_PACKET_DECISION.json").write_text(json.dumps(final_decision, indent=2), encoding="utf-8")

    # Meeting note.
    lines = []
    lines.append("# Reza meeting packet: honest final audit")
    lines.append("")
    lines.append("## Bottom line")
    lines.append("")
    lines.append("The C4 result should not be presented as fully locked under Reza's requested full-control screen. It remains strong in benchmark, crop-free, natural-grassland-like, and no-VPD sensitivity models, but it does not survive the full climate/environment model once baseline VPD is included.")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Reza concern status table")
    lines.append("")
    lines.append("```text")
    ...
    note = "\n".join(lines)
    (TXT / "REZA_HONEST_MEETING_PACKET.md").write_text(note, encoding="utf-8")
...
Akul
"""
    (TXT / "REZA_REPLY_HONEST_FINAL.md").write_text(reply, encoding="utf-8")

    print("===== FINAL HONEST DECISION =====")
    print(json.dumps(final_decision, indent=2))
    print("")
    print("===== REZA CONCERN STATUS TABLE =====")
    ...
    print("")
    print("===== REZA MEETING PACKET =====")
    print(note)
    print("")
    print("===== REZA REPLY =====")
    print(reply)
    print("")
    print("WROTE", TAB / "STAGE1B6AZ_HONEST_REZA_PACKET_DECISION.json")
    print("WROTE", TAB / "Table_PRODUCT03fr_key_c4_result_summary.csv")
    print("WROTE", TAB / "Table_PRODUCT03fs_vpd_killer_diagnostic.csv")
    print("WROTE", TAB / "Table_PRODUCT03ft_reza_concern_status_table.csv")
    print("WROTE", TXT / "REZA_HONEST_MEETING_PACKET.md")
    print("WROTE", TXT / "REZA_REPLY_HONEST_FINAL.md")
