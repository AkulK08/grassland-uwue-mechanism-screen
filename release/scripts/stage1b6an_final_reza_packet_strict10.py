...
OUT = Path("results/stage1b6an_final_reza_packet_strict10")
TAB = OUT / "tables"
TXT = OUT / "text"
for p in [TAB, TXT]:
    p.mkdir(parents=True, exist_ok=True)

QUALITY = Path("results/stage1b6am_raw_tower_download_ingest/tables/Table_PRODUCT03bv_reza_tower_table_raw_quality_completed.csv")
...
c4_primary = c4[(c4["term"] == "c4_fraction") & (c4.get("primary_controlled_reza_pass", False).astype(str).str.lower() == "true")] if len(c4) else pd.DataFrame()
...
# Decide whether Reza is fully satisfied.
...
satisfaction = pd.DataFrame([
    {
        "reza_item": "Product identifiability quantified",
        "status": "SATISFIED",
        "evidence": "Product anomaly correlations computed for ET, GPP, WUE, uWUE, log-WUE, log-uWUE.",
    },
    {
        "reza_item": "Product-screened definition documented",
        "status": "SATISFIED",
        "evidence": screen.iloc[0].get("answer_for_reza", "Product-screened definition table exists."),
    },
    {
        "reza_item": "Tower validation table exists",
        "status": "SATISFIED",
        "evidence": f"Tower-satellite agreement table exists with {agree['site_id'].nunique()} total sites and {len(agree)} site-product rows.",
    },
    {
        "reza_item": "Tower quality filters applied",
        "status": tower_status,
...
satisfaction.to_csv(TAB / "Table_PRODUCT03cb_final_reza_satisfaction_matrix_strict10.csv", index=False)

decision = {
    "generated": datetime.now().isoformat(timespec="seconds"),
    "stage": "1B.6AN_final_reza_packet_strict10",
    ...
    "can_send_to_reza": tower_status != "NOT_SATISFIED_RAW_QUALITY_STILL_MISSING",
    "honest_limitation": (
...
(TAB / "STAGE1B6AN_FINAL_REZA_PACKET_STRICT10_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")
...
(TXT / "REZA_READY_RESPONSE_STRICT10.md").write_text("\n".join(email), encoding="utf-8")

report = []
report.append("# Stage 1B.6AN final Reza packet strict-10")
report.append("")
...
(TXT / "STAGE1B6AN_FINAL_REZA_PACKET_STRICT10_REPORT.md").write_text("\n".join(report), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "STAGE1B6AN_FINAL_REZA_PACKET_STRICT10_DECISION.json")
print("WROTE", TXT / "REZA_READY_RESPONSE_STRICT10.md")
print("WROTE", TXT / "STAGE1B6AN_FINAL_REZA_PACKET_STRICT10_REPORT.md")
