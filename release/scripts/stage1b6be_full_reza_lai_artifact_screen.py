...
ROOT = Path.cwd()
OUT = ROOT / "results/stage1b6be_full_reza_lai_artifact_screen"
TAB = OUT / "tables"
TXT = OUT / "text"
for p in [TAB, TXT]:
...
scenario_masks = {
    "all_points_full_reza_controls": pd.Series(True, index=d.index),
...
for scenario, mask in scenario_masks.items():
    ...
    model_rows += compare(
        sub,
        f"{scenario}__LAI_main_FULL_REZA",
        main_full,
        main_reduced,
        ["lai_z"],
        note="Full Reza-style controls: VPD, aridity, MAT, precipitation, soil moisture, soil texture PC1, lat/lon. LAI is focal predictor."
    )

    model_rows += compare(
        sub,
        f"{scenario}__LAI_x_temperature_FULL_REZA",
        int_full,
        int_reduced,
        ["lai_z:mat_z"],
        note="Full Reza-style interaction model with soil texture and hydroclimate/geography controls."
    )

    if gs_int_full is not None:
        model_rows += compare(
            sub,
            f"{scenario}__LAI_x_growing_season_temperature_FULL_REZA",
            gs_int_full,
            gs_int_reduced,
            ["lai_z:gs_temp_z"],
...
for label, formula, term in [
    ("all_points_full_reza_controls__LAI_main_FULL_REZA", main_full, "lai_z"),
    ("all_points_full_reza_controls__LAI_x_temperature_FULL_REZA", int_full, "lai_z:mat_z"),
]:
...
full_models.to_csv(TAB / "FULL_REZA_CONTROL_LAI_MODELS.csv", index=False)
...
pd.DataFrame(vif_rows).to_csv(TAB / "FULL_REZA_VIF_DIAGNOSTICS.csv", index=False)
...
main_all = row_lookup(full_models, "all_points_full_reza_controls__LAI_main", "lai_z")
int_all = row_lookup(full_models, "all_points_full_reza_controls__LAI_x_temperature", "lai_z:mat_z")
...
if decision["all_points_LAI_full_control_pass"] and decision["all_points_LAIxTemp_full_control_pass"]:
    decision["reza_style_verdict"] = "CONDITIONAL_PASS_AS_REGIME_DEPENDENCE_RESULT_NOT_UNIVERSAL_TRAIT_CAUSALITY"
    ...
    decision["reza_style_verdict"] = "FAIL_OR_INCOMPLETE_FOR_MAIN_CLAIM"
    decision["recommended_claim"] = (
        "Do not make LAI the central claim unless the full-control and artifact-screen models pass."
    )

(TAB / "REZA_STYLE_PASS_FAIL_DECISION.json").write_text(json.dumps(decision, indent=2))
...
memo = []
memo.append("Stage1B6BE full Reza-style LAI artifact screen")
memo.append("=" * 90)
memo.append("")
...
memo.append("")
memo.append("Reza-style decision:")
memo.append(json.dumps(decision, indent=2))
memo.append("")
memo.append("Full-control LAI models:")
memo.append(show("FULL_REZA_CONTROL_LAI_MODELS.csv", 80))
memo.append("")
...
memo.append(show("FULL_REZA_VIF_DIAGNOSTICS.csv", 30))
memo.append("")
memo.append("Important files:")
for f in [
    "REZA_STYLE_PASS_FAIL_DECISION.json",
    "FULL_REZA_CONTROL_LAI_MODELS.csv",
    ...
    "FULL_REZA_VIF_DIAGNOSTICS.csv",
]:
    memo.append(f"- {TAB / f}")

(TXT / "READ_ME_full_reza_lai_artifact_screen.txt").write_text("\n".join(memo))

print("\nDONE.")
print(f"Outputs written to: {OUT}")
print("\nPaste this back:")
print(f"cat {TXT / 'READ_ME_full_reza_lai_artifact_screen.txt'}")
