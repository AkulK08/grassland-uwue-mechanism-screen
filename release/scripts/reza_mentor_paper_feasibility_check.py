...
OUT = Path("results/mentor_paper_feasibility")
...
add("# Mentor-paper feasibility check")
add("")
...
if gate12_ok and slope_available and trait_any_ok:
    add("The mentor-style paper is possible in principle, but still conditional on tower validation. The raw data support moving toward a trait-response paper, not a universal breakdown paper.")
elif gate12_ok and not trait_any_ok:
    add("The response pipeline is strong, but the trait endpoint may be underpowered/sparse with only 199 points. If tower validation succeeds, you may still need a larger trait-covered satellite sample before the exact mentor thesis is defensible.")
else:
    add("The exact mentor-style paper is not yet supported by the available results.")

Path(OUT / "README_mentor_paper_feasibility.md").write_text("\n".join(report))
...
with open(OUT / "mentor_paper_feasibility_verdict.json", "w") as f:
    json.dump(summary, f, indent=2)

add("")
add(f"WROTE `{OUT / 'README_mentor_paper_feasibility.md'}`")
add(f"WROTE `{OUT / 'mentor_paper_feasibility_verdict.json'}`")
