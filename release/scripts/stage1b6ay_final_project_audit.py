...
ROOT = Path.cwd()
OUT = ROOT / "results/stage1b6ay_final_project_audit"
TAB = OUT / "tables"
TXT = OUT / "text"
    ...
    for root in [ROOT / "results", ROOT / "data"]:
        ...
        for p in root.rglob("*.csv"):
            ...
            if "_project_raw_exports" in sp:
                continue
            if "stage1b6ay_final_project_audit" in sp:
                continue
...
def main():
    ...
    # If no clean productivity exists, project's full request cannot be marked solved.
    ...
    decision = {
        "generated": now(),
        "stage": "1B.6AY_final_project_audit",
        ...
        "all_project_concerns_completely_solved": bool(worry1_pass and worry2_pass and worry3_pass),
    ...
    if decision["all_project_concerns_completely_solved"]:
        ...
        decision["honest_interpretation"] = "C4 survives climate/soil/baseline-moisture controls and crop/natural-grassland filtering, but not the full model with clean productivity if available. Do not claim all project concerns are solved; frame as sensitivity-dependent or ask whether productivity is overcontrol/proxy."
    else:
        decision["honest_interpretation"] = "C4 does not survive the decisive full-control screen. Do not frame the paper as a clean C4 mechanism unless project accepts a narrower sensitivity model."

    (TAB / "STAGE1B6AY_FINAL_project_AUDIT_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")

    note = []
    note.append("# Final project audit note")
    note.append("")
    ...
    note_text = "\n".join(note)
    (TXT / "FINAL_project_AUDIT_NOTE.md").write_text(note_text, encoding="utf-8")
...
Akul
"""
    (TXT / "project_REPLY_FINAL_AUDIT.md").write_text(reply, encoding="utf-8")
    ...
    print(reply)
    print("")
    print("WROTE", TAB / "STAGE1B6AY_FINAL_project_AUDIT_DECISION.json")
    ...
    print("WROTE", TXT / "FINAL_project_AUDIT_NOTE.md")
    print("WROTE", TXT / "project_REPLY_FINAL_AUDIT.md")
