...
ROOT = Path.cwd()
OUT = ROOT / "results/stage1b6ax_resolve_all_project_concerns"
TAB = OUT / "tables"
TXT = OUT / "text"
...
STRICT_project_SITES = ["CA-SF3", "RU-NeC", "US-CMW", "US-Cop", "US-Dk1", "US-SP1", "US-Ton", "US-Var"]
SENS_project_SITES = ["CA-SF3", "CN-HaM", "RU-NeC", "US-CMW", "US-Cop", "US-Dk1", "US-SP1", "US-Ton", "US-Var"]
    ...
    for root in [ROOT / "results", ROOT / "data"]:
        ...
        for p in root.rglob("*.csv"):
            ...
            if "_project_raw_exports" in sp:
                continue
            if "stage1b6ax_resolve_all_project_concerns" in sp:
                continue
...
def main():
    ...
    # Primary project full control set.
    primary_controls = [
    ...
    model_specs = {
        ...
        "FULL_STRICT_controls_texture_pc1": primary_controls,
        "FULL_STRICT_controls_raw_texture": raw_texture_controls,
    ...
    models.to_csv(TAB / "Table_PRODUCT03et_project_full_control_c4_models.csv", index=False)
    missingness.to_csv(TAB / "Table_PRODUCT03eu_project_full_control_missingness.csv", index=False)
    ...
    c4_rows.to_csv(TAB / "Table_PRODUCT03ev_project_c4_rows_only.csv", index=False)
    ...
    # Pass/fail logic.
    full_label_natural = "natural_grassland_like_no_crop_points__FULL_STRICT_controls_texture_pc1"
    full_label_no_crop = "no_crop_flagged_points__FULL_STRICT_controls_texture_pc1"
    ...
    worry2_crop_pass = bool(
        model_passes(models, "no_crop_flagged_points__FULL_STRICT_controls_texture_pc1", c4)
        or model_passes(models, "no_crop_flagged_points__FULL_STRICT_controls_raw_texture", c4)
    )
    worry2_natural_pass = bool(
        model_passes(models, "natural_grassland_like_no_crop_points__FULL_STRICT_controls_texture_pc1", c4)
        or model_passes(models, "natural_grassland_like_no_crop_points__FULL_STRICT_controls_raw_texture", c4)
    )
    ...
    decision = {
        "generated": now(),
        "stage": "1B.6AX_resolve_all_project_concerns",
        ...
        "all_project_concerns_resolved": bool(worry1_pass and worry2_pass and worry3_pass),
    ...
    (TAB / "STAGE1B6AX_RESOLVE_ALL_project_CONCERNS_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")

    # Meeting note.
    lines = []
    lines.append("# project meeting note: all requested checks")
    lines.append("")
    ...
    report = "\n".join(lines)
    (TXT / "project_ALL_CONCERNS_RESOLUTION_NOTE.md").write_text(report, encoding="utf-8")

    # Short direct reply to project if all pass.
    if decision["all_project_concerns_resolved"]:
        reply = """Hi project,
...
"""
    ...
    (TXT / "project_REPLY_AFTER_ALL_CHECKS.md").write_text(reply, encoding="utf-8")
    ...
    print(reply)
    print("")
    print("WROTE", TAB / "STAGE1B6AX_RESOLVE_ALL_project_CONCERNS_DECISION.json")
    print("WROTE", TAB / "Table_PRODUCT03et_project_full_control_c4_models.csv")
    print("WROTE", TAB / "Table_PRODUCT03ev_project_c4_rows_only.csv")
    ...
    print("WROTE", TXT / "project_ALL_CONCERNS_RESOLUTION_NOTE.md")
    print("WROTE", TXT / "project_REPLY_AFTER_ALL_CHECKS.md")
