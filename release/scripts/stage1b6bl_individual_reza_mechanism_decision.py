...
OUT = ROOT / "results" / "stage1b6bl_individual_reza_mechanism_decision"
TAB = OUT / "tables"
TXT = OUT / "text"
FIG = OUT / "figures"
...
def score_for_dedup(row):
    score = 0
    score += 1000 * int(boolish(row.get("FULL_REZA_STRICT_PASS")))
...
def main():
    ...
    full = gates[gates["FULL_REZA_STRICT_PASS"]].copy()
    ...
    full.to_csv(TAB / "FULL_REZA_STRICT_PASS_NO_C4_FOCUS.csv", index=False)
    sat.to_csv(TAB / "SATELLITE_REZA_PASS_NO_TOWER_NO_C4_FOCUS.csv", index=False)
    ...
    summary = {
        ...
        "n_full_reza_strict_pass_no_c4_focus": int(len(full)),
        "n_satellite_reza_pass_no_tower_no_c4_focus": int(len(sat)),
        "n_one_gate_short_no_c4_focus": int(len(tier2)),
        "underlying_audit": audit,
    }
    with open(TAB / "INDIVIDUAL_REZA_DECISION_PROGRAMMING_AUDIT.json", "w") as f:
        json.dump(summary, f, indent=2)

    lines = []
    lines.append("Stage1B6BL individual-feature Reza mechanism decision")
    lines.append("=" * 88)
    lines.append("")
    ...
    lines.append(f"- FULL_REZA_STRICT_PASS including tower: {len(full)}")
    lines.append(f"- SATELLITE_REZA_PASS_NO_TOWER: {len(sat)}")
    lines.append(f"- ONE_GATE_SHORT non-tower near-passes: {len(tier2)}")
    lines.append("")
    lines.append("Full Reza-strict passes")
    if full.empty:
        lines.append("- NONE")
    ...
    lines.append("")
    lines.append("Satellite-side Reza passes, no tower")
    if sat.empty:
        lines.append("- NONE")
    else:
        cols = [
    ...
    for p in [
        TAB / "FULL_REZA_STRICT_PASS_NO_C4_FOCUS.csv",
        TAB / "SATELLITE_REZA_PASS_NO_TOWER_NO_C4_FOCUS.csv",
        ...
        TAB / "INDIVIDUAL_REZA_DECISION_PROGRAMMING_AUDIT.json",
    ]:
        lines.append(f"- {p}")

    readme = "\n".join(lines)
    (TXT / "READ_ME_individual_reza_mechanism_decision.txt").write_text(readme)

    print("DONE.")
    print(f"Outputs written to: {OUT}")
    print("")
    print("Paste this back:")
    print(f"cat {TXT / 'READ_ME_individual_reza_mechanism_decision.txt'}")
