...
def fit_latent_additive(
    ...
    for f in families:
        fitted = np.asarray(fitted, dtype=float).copy() + work[f].map(effects[f]).fillna(0.0).astype(float).values
...
def fit_latent_suite(obs, weight_col=None, n_boot = 50, n_iter=30, boot_iter=20, label="equal"):
    work = obs.copy()
