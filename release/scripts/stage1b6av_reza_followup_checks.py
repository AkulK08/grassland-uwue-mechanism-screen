...
def main():
    ...
    decision = {
        "generated": now(),
        "stage": "1B.6AV_reza_followup_checks",
        "response_variable": response_col,
        "c4_variable": c4_col,
        "required_controls_requested_by_reza": wanted_roles,
    ...
    (TAB / "STAGE1B6AV_REZA_FOLLOWUP_CHECKS_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")

    # Meeting note
    lines = []
    lines.append("# Reza meeting note: follow-up checks")
    lines.append("")
    ...
    if full_control_complete:
        ...
        lines.append("The current full-available model should therefore be treated as a check, not as the final full-control model Reza requested.")
    lines.append("")
    ...
    meeting_note = "\n".join(lines)
    (TXT / "REZA_MEETING_NOTE_FOLLOWUP_CHECKS.md").write_text(meeting_note, encoding="utf-8")

    # Short email/reply draft if useful
    reply = []
    reply.append("Hi Reza,")
    reply.append("")
    ...
    reply.append("Best,")
    reply.append("Akul")
    (TXT / "REZA_SHORT_REPLY.md").write_text("\n".join(reply), encoding="utf-8")
    ...
    print("\n".join(reply))
    print("")
    print("WROTE", TAB / "STAGE1B6AV_REZA_FOLLOWUP_CHECKS_DECISION.json")
    ...
    print("WROTE", TXT / "REZA_MEETING_NOTE_FOLLOWUP_CHECKS.md")
    print("WROTE", TXT / "REZA_SHORT_REPLY.md")
