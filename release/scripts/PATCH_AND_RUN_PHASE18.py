...
markers = [
    '    recommendation = f"""# Phase 18 grassland + spatial/trait lock',
    '    recommendation = """# Phase 18 grassland + spatial/trait lock',
    '    recommendation = f"""',
]
...
if cut is None:
    raise SystemExit("Could not find broken recommendation block. Run: tail -100 scripts/phase18_grassland_spatial_trait_lock.py")
...
tail = r'''
    ...
    top_tree_txt = tree_text if str(tree_text).strip() else "No decision-tree rules available."
    ...
    recommendation_lines = [
        ...
        "",
        "Nearest satellite traits are only provisional proxies unless the nearest satellite point is actually close to the tower. The manuscript-grade step remains tower-centered satellite and trait extraction at the tower coordinates.",
        "",
    ...
    save_text(recommendation, TXT / "PHASE18_GRASSLAND_SPATIAL_TRAIT_LOCK_RECOMMENDATION.md")
    ...
    manifest = {
        "all_tower_sites": all_n,
        "strict_grassland_GRA_sites": strict_n,
        "expanded_grassland_savanna_open_sites": expanded_n,
        "open_nonforest_sites": open_n,
        "all_satbreak_fraction": all_sat,
        "strict_grassland_satbreak_fraction": strict_sat,
        "expanded_grassland_savanna_satbreak_fraction": expanded_sat,
        "open_nonforest_satbreak_fraction": open_sat,
        "satellite_trait_proxy_source": sat_source,
        "outputs": {
            "tower_annotation": str(TAB / "Table101_tower_landcover_spatial_trait_annotation.csv"),
            "scope_summary": str(TAB / "Table102_tower_response_summary_by_validation_scope.csv"),
            "class_counts": str(TAB / "Table103_tower_response_class_counts_by_scope.csv"),
            "spatial_enrichment": str(TAB / "Table104_spatial_ecoregion_landcover_enrichment.csv"),
            "trait_characteristic_tests": str(TAB / "Table105_characteristic_trait_association_tests.csv"),
            "candidate_rules": str(TAB / "Table106_candidate_characteristic_rules_for_tower_satbreak.csv"),
            "decision_tree_importance": str(TAB / "Table107_decision_tree_feature_importance.csv"),
            "target_manifest": str(TAB / "Table109_satellite_extraction_target_files_manifest.csv"),
            "high_priority_targets": str(TAB / "Table110_high_priority_grassland_open_tower_sites_for_satellite_extraction.csv"),
            "recommendation": str(TXT / "PHASE18_GRASSLAND_SPATIAL_TRAIT_LOCK_RECOMMENDATION.md"),
        },
    }

    (OUT / "phase18_grassland_spatial_trait_manifest.json").write_text(
    ...
    print(TAB / "Table109_satellite_extraction_target_files_manifest.csv")
    print(TAB / "Table110_high_priority_grassland_open_tower_sites_for_satellite_extraction.csv")
    print(TXT / "PHASE18_GRASSLAND_SPATIAL_TRAIT_LOCK_RECOMMENDATION.md")
