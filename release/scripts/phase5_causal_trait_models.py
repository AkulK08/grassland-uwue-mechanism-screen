...
def pbar(iterable, **kwargs):
    """
    Safe progress-bar wrapper.
    If tqdm is unavailable, falls back to the original iterable.
    """
    if tqdm is None:
        return iterable
...
def run_ols_framework(df, sets):
    ...
    for outcome in pbar(
        outcomes,
        total=len(outcomes),
        desc="PHASE 5 DML outcomes",
        unit="outcome",
    ):
        print(f"[DML] outcome={outcome}", flush=True)
        model_role = (
...
def run_single_ml_model(df, outcome, predictors, categorical, predictor_set, model_family, model):
    ...
    for fold, (tr, te) in enumerate(pbar(
        cv_splits,
        total=len(cv_splits),
        desc=f"CV folds {model_family} {outcome} {predictor_set}",
        unit="fold",
        leave=False,
    )):
...
def run_ml_framework(df, sets):
    ...
    total_ml_tasks = len(outcomes) * len(predictor_sets) * len(model_specs)
    ml_tasks = []
    for outcome in outcomes:
        for predictor_set, preds, cat_cols in predictor_sets:
            for model_family, model in model_specs:
                ml_tasks.append((outcome, predictor_set, preds, cat_cols, model_family, model))

    for outcome, predictor_set, preds, cat_cols, model_family, model in pbar(
        ml_tasks,
        total=total_ml_tasks,
        desc="PHASE 5 ML total",
        unit="model",
    ):
        print(f"[ML] outcome={outcome} predictor_set={predictor_set} model={model_family}", flush=True)
...
def run_product_family_sensitivity(df, sets, ols_df, dml_df):
    ...
    product_family_outcomes = sets["outcomes"]["product_family"]
    for outcome in pbar(
        product_family_outcomes,
        total=len(product_family_outcomes),
        desc="PHASE 5 product-family sensitivity",
        unit="outcome",
    ):
        print(f"[PRODUCT FAMILY] outcome={outcome}", flush=True)
