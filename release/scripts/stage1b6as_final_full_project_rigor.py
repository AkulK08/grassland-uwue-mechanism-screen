...
ROOT = Path.cwd()
MANUAL = ROOT / "data/raw/towers/_project_raw_exports/manual_fluxnet"
OLD_EXTRACTED = ROOT / "data/raw/towers/_project_raw_exports/extracted"
FRESH_EXTRACTED = ROOT / "data/raw/towers/_project_raw_exports/extracted_final_FULL_STRICT"

OUT = ROOT / "results/stage1b6as_final_FULL_STRICT_rigor"
TAB = OUT / "tables"
TXT = OUT / "text"
...
AGREE = ROOT / "results/stage1b6ak_project_complete_resolution_packet/tables/Table_PRODUCT03bg_project_tower_satellite_agreement_long.csv"
SCREEN = ROOT / "results/stage1b6ak_project_complete_resolution_packet/tables/Table_PRODUCT03be_product_screened_definition_final.csv"
PROD = ROOT / "results/stage1b6ak_project_complete_resolution_packet/tables/Table_PRODUCT03bd_product_identifiability_summary.csv"
C4 = ROOT / "results/stage1b6ak_project_complete_resolution_packet/tables/Table_PRODUCT03bm_c4_project_decision_by_model.csv"
MIXED = ROOT / "results/stage1b6ak_project_complete_resolution_packet/tables/Table_PRODUCT03bl_c4_mixedlm_ecoregion_random_intercept.csv"
...
audit.to_csv(TAB / "Table_PRODUCT03de_FULL_STRICT_column_audit.csv", index=False)
allq.to_csv(TAB / "Table_PRODUCT03df_FULL_STRICT_all_site_year_candidates.csv", index=False)
...
best.to_csv(TAB / "Table_PRODUCT03dg_FULL_STRICT_best_site_year_quality.csv", index=False)
...
for site in TARGET:
    ...
    if not has_any:
        ...
        status = "COMPUTED_BUT_FAILS_project_FILTER"
...
summary.to_csv(TAB / "Table_PRODUCT03dh_final_FULL_STRICT_site_status.csv", index=False)
...
strict_rank.to_csv(TAB / "Table_PRODUCT03di_final_FULL_STRICT_strict_et_ranking.csv", index=False)
sensitivity_rank.to_csv(TAB / "Table_PRODUCT03dj_final_FULL_STRICT_sensitivity_et_ranking.csv", index=False)
...
if len(focal):
    primary = focal.get("primary_controlled_project_pass", pd.Series(False, index=focal.index)).astype(str).str.lower().eq("true").any()
...
satisfaction = pd.DataFrame([
    ...
    {
        "project_item": "Closure + gap-fill computable",
    ...
    {
        "project_item": "Quality-filtered tower ranking",
    ...
    {
        "project_item": "C3/C4 pre-specified test",
        "status": "SATISFIED",
...
satisfaction.to_csv(TAB / "Table_PRODUCT03dk_final_FULL_STRICT_satisfaction.csv", index=False)

decision = {
    "generated": now(),
    "stage": "1B.6AS_final_FULL_STRICT_rigor",
    "target_sites_n": 13,
    ...
    "fully_satisfies_every_project_raw_tower_requirement": bool(full_raw_13),
    "can_send_FULL_STRICT_without_caveat": bool(can_send_no_caveat),
    "can_send_project_as_full_quality_filtered_rigor": bool(can_send_quality_filtered),
...
(TAB / "STAGE1B6AS_FINAL_FULL_STRICT_RIGOR_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")
...
(TXT / "project_READY_RESPONSE_FINAL_FULL_RIGOR.md").write_text("\n".join(email), encoding="utf-8")

report = []
report.append("# Stage 1B.6AS final full strict rigor")
report.append("")
...
(TXT / "STAGE1B6AS_FINAL_FULL_STRICT_RIGOR_REPORT.md").write_text("\n".join(report), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "STAGE1B6AS_FINAL_FULL_STRICT_RIGOR_DECISION.json")
print("WROTE", TAB / "Table_PRODUCT03de_FULL_STRICT_column_audit.csv")
print("WROTE", TAB / "Table_PRODUCT03dh_final_FULL_STRICT_site_status.csv")
print("WROTE", TAB / "Table_PRODUCT03di_final_FULL_STRICT_strict_et_ranking.csv")
print("WROTE", TAB / "Table_PRODUCT03dj_final_FULL_STRICT_sensitivity_et_ranking.csv")
print("WROTE", TAB / "Table_PRODUCT03dk_final_FULL_STRICT_satisfaction.csv")
print("WROTE", TXT / "project_READY_RESPONSE_FINAL_FULL_RIGOR.md")
print("WROTE", TXT / "STAGE1B6AS_FINAL_FULL_STRICT_RIGOR_REPORT.md")
