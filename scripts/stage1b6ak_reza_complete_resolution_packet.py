...
OUT = Path("results/stage1b6ak_reza_complete_resolution_packet")
TAB = OUT / "tables"
TXT = OUT / "text"
FIG = OUT / "figures"
...
C4_JOINED = Path("results/stage1b6ai_reza_final_lock_with_c4/tables/Table_PRODUCT03aq_c4_sampled_point_table.csv")
C4_CLEAN = Path("results/stage1b6aj_clean_c4_model_lock/tables/Table_PRODUCT03bb_clean_c4_decision_by_response.csv")
SCREEN_ANSWER = Path("results/stage1b6ai_reza_final_lock_with_c4/tables/Table_PRODUCT03al_product_screening_answer_for_reza.csv")
...
if SCREEN_ANSWER.exists():
    ...
    screen = pd.DataFrame([{
        ...
        "answer_for_reza": "Product-screening audit file missing."
    }])
...
closure_path = Path("results/stage1b6ai_reza_final_lock_with_c4/tables/Table_PRODUCT03am_tower_flux_quality_closure_gapfill.csv")
...
tower_final.to_csv(TAB / "Table_PRODUCT03bf_reza_final_tower_validation_table.csv", index=False)
...
agreement_long.to_csv(TAB / "Table_PRODUCT03bg_reza_tower_satellite_agreement_long.csv", index=False)
...
et_rank.to_csv(TAB / "Table_PRODUCT03bh_reza_final_et_product_ranking.csv", index=False)
...
if len(focal):
    ...
    focal["primary_controlled_reza_pass"] = (
    ...
    focal = focal.sort_values(
        ["primary_controlled_reza_pass","sensitivity_controlled_pass","exploratory_minimal_pass","p_normal_approx"],
        ascending=[False,False,False,True]
    )
focal.to_csv(TAB / "Table_PRODUCT03bm_c4_reza_decision_by_model.csv", index=False)
...
items.append(status_item(
    ...
    screen.iloc[0].get("answer_for_reza", "No answer available."),
...
items.append(status_item(
    ...
    output_file="Table_PRODUCT03bf_reza_final_tower_validation_table.csv"
))
...
items.append(status_item(
    ...
    "Raw tower files with H, LE, NETRAD, G and FLUXNET/AmeriFlux QC fields are not present/parseable for the clean 13-site table. This must be stated to Reza or filled by downloading raw network exports.",
    output_file="Table_PRODUCT03bf_reza_final_tower_validation_table.csv"
))
...
items.append(status_item(
    ...
    output_file="Table_PRODUCT03bh_reza_final_et_product_ranking.csv"
))
...
items.append(status_item(
    ...
    output_file="Table_PRODUCT03bm_c4_reza_decision_by_model.csv"
))

if len(focal):
    primary_pass = bool(focal["primary_controlled_reza_pass"].any())
...
items.append(status_item(
    ...
    "If only exploratory/minimal C4 passes, report it as supportive/exploratory, not as the primary controlled Reza hypothesis.",
    output_file="Table_PRODUCT03bm_c4_reza_decision_by_model.csv"
))
...
satisfaction.to_csv(TAB / "Table_PRODUCT03bn_reza_protocol_satisfaction_matrix.csv", index=False)
...
decision = {
    "generated": now(),
    "stage": "1B.6AK_Reza_complete_resolution_packet",
    ...
    "product_screened_answer": screen.iloc[0].get("answer_for_reza", "") if not screen.empty else "",
    ...
    "all_reza_items_fully_satisfied": bool((satisfaction["status"] == "SATISFIED").all()),
...
(TAB / "STAGE1B6AK_REZA_COMPLETE_RESOLUTION_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")

# Reza-ready email content.
reza_lines = []
reza_lines.append("Hi Reza,")
reza_lines.append("")
reza_lines.append("Thank you again — I treated your note as an analysis-locking checklist rather than as a manuscript-writing prompt, and I reran the work around the gates you specified.")
reza_lines.append("")
reza_lines.append("First, I audited what I meant by “product-screened.” I did not find evidence that pixels were filtered by product agreement. The audit found 0 agreement-filter hits and 208 QC/product-quality hits, so I am defining product-screened as QC/product-confidence screening rather than retaining only product-agreeing pixels.")
reza_lines.append("")
reza_lines.append(f"Second, I rebuilt the tower-validation table. The current clean table has {len(tower_final)} tower sites and {len(agreement_long)} site-product rows. The ET-product ranking currently selects {tower_ranked_et}. I am treating tower agreement as a confidence/ranking layer, not as proof of a threshold.")
reza_lines.append("")
if closure_nonmissing == 0 or gap_nonmissing == 0:
    reza_lines.append("The one remaining caveat is that energy-balance closure and gap-fill fields are not yet computable for the clean tower-validation rows from the local raw files I currently have. I generated the table with explicit missing-field flags rather than filling these values artificially. If you want those filters strictly enforced before submission, the next mechanical step is to add the raw FLUXNET/AmeriFlux exports containing H, LE, NETRAD, G, and QC fields for the selected towers, after which the same script will populate closure and gap-fill columns.")
    reza_lines.append("")
else:
    reza_lines.append("The tower table also includes closure and gap-fill fields, so the tower quality-filter requirement is filled directly.")
    reza_lines.append("")
reza_lines.append("Third, I downloaded and attached the Luo et al. C4 composition layer and ran the C3/C4 test. I separated the primary controlled test from exploratory/minimal screens so we do not overclaim.")
reza_lines.append("")
reza_lines.append(best_c4_text)
reza_lines.append("")
if paper_fork == "ECOLOGICAL_C3C4_MECHANISM_PRIMARY_SUPPORTED":
    reza_lines.append("The primary controlled C4 test supports the ecological mechanism framing.")
elif paper_fork == "ECOLOGICAL_C3C4_MECHANISM_SENSITIVITY_SUPPORTED":
    reza_lines.append("The primary controlled C4 test is not the cleanest positive result, but the pre-specified sensitivity response supports the C4 mechanism.")
elif paper_fork == "C3C4_EXPLORATORY_SIGNAL_ONLY_METHODS_OR_MECHANISM_NEEDS_CAUTION":
    reza_lines.append("The C4 signal is real enough to keep as a mechanism candidate, but the strongest positive row is exploratory/minimal rather than the primary controlled model, so I would not overstate it.")
else:
    reza_lines.append("The C4 test does not support the ecological mechanism framing; the safer paper fork is the product-identifiability/tower-ranking paper.")
reza_lines.append("")
reza_lines.append("I also generated product-anomaly correlation tables so the product disagreement/identifiability issue is quantified rather than assumed away.")
reza_lines.append("")
reza_lines.append("Best,")
reza_lines.append("Akul")

(TXT / "REZA_READY_RESPONSE_DRAFT.md").write_text("\n".join(reza_lines), encoding="utf-8")

# Report.
report = []
report.append("# Stage 1B.6AK Reza complete resolution packet")
report.append("")
...
report.append("```")
report.append("")
report.append("## Reza protocol satisfaction matrix")
report.append("")
report.append("```text")
...
report.append("```")
report.append("")
report.append("## C4 Reza decision table")
report.append("")
report.append("```text")
...
report.append("")
if decision["all_reza_items_fully_satisfied"]:
    report.append("All Reza items are fully satisfied from local data.")
else:
    report.append("The analysis packet is complete, but not every Reza item is fully satisfied from local data. The unresolved items are explicitly flagged in the satisfaction matrix, especially closure/gap-fill if raw tower energy-balance/QC fields are unavailable.")
report.append("")
report.append("## Reza-ready response draft")
report.append("")
report.append("```text")
report.append("\n".join(reza_lines))
report.append("```")
...
report_text = "\n".join(report)
(TXT / "STAGE1B6AK_REZA_COMPLETE_RESOLUTION_REPORT.md").write_text(report_text, encoding="utf-8")
...
if plt is not None:
    ...
    try:
        if len(focal):
            ...
            plt.tight_layout()
            plt.savefig(FIG / "Figure_PRODUCT03g_c4_reza_decision_rows.png", dpi=220)
            plt.close()
    except Exception:
        pass
...
print("WROTE", TAB / "Table_PRODUCT03bf_reza_final_tower_validation_table.csv")
print("WROTE", TAB / "Table_PRODUCT03bg_reza_tower_satellite_agreement_long.csv")
print("WROTE", TAB / "Table_PRODUCT03bh_reza_final_et_product_ranking.csv")
...
print("WROTE", TAB / "Table_PRODUCT03bm_c4_reza_decision_by_model.csv")
print("WROTE", TAB / "Table_PRODUCT03bn_reza_protocol_satisfaction_matrix.csv")
print("WROTE", TAB / "STAGE1B6AK_REZA_COMPLETE_RESOLUTION_DECISION.json")
print("WROTE", TXT / "REZA_READY_RESPONSE_DRAFT.md")
print("WROTE", TXT / "STAGE1B6AK_REZA_COMPLETE_RESOLUTION_REPORT.md")
