...
OUT = Path("results/stage1b6k_parallel_reviewer_readiness")
TAB = OUT / "tables"
TXT = OUT / "text"
...
add("Tower-vs-satellite arbitration ready", "tower_validation",
    ...
    True,
    "Existing GOSIF/GLEAM partial validation is not enough for final reviewer claim.")
...
add("Final gridded MODIS xarray/Zarr cube", "optional_gridded_extension",
    ...
    False,
    "Only required for mapped/global pixel claims, not for tower-centered reviewer validation.")

df = pd.DataFrame(checks)
df.to_csv(TAB / "Table_PRODUCT02bk_parallel_reviewer_requirement_readiness.csv", index=False)

summary = df.groupby(["category", "status"]).size().reset_index(name="n")
summary.to_csv(TAB / "Table_PRODUCT02bl_parallel_reviewer_readiness_summary.csv", index=False)
...
report = []
report.append("# Stage 1B.6K parallel reviewer-readiness prep")
report.append("")
...
report.append("```")
report.append("")
report.append("## Full reviewer requirement table")
report.append("")
report.append("```text")
...
report.append("")
report.append("Do not make the final full reviewer-standard claim until all blocking rows are resolved. The gridded MODIS cube is not blocking unless the claim becomes spatially gridded/global.")
report.append("")

(TXT / "STAGE1B6K_PARALLEL_reviewer_READINESS_REPORT.md").write_text("\n".join(report), encoding="utf-8")

machine = {
    "stage": "1B.6K_parallel_reviewer_readiness",
    "status": "complete",
    "n_requirements": int(len(df)),
    "n_blockers": int(len(blocking)),
    "outputs": {
        "readiness": str(TAB / "Table_PRODUCT02bk_parallel_reviewer_requirement_readiness.csv"),
        "summary": str(TAB / "Table_PRODUCT02bl_parallel_reviewer_readiness_summary.csv"),
        "blockers": str(TAB / "Table_PRODUCT02bm_current_blockers_for_final_claim.csv"),
        "report": str(TXT / "STAGE1B6K_PARALLEL_reviewer_READINESS_REPORT.md"),
    }
}

(TAB / "STAGE1B6K_PARALLEL_reviewer_READINESS_SUMMARY.json").write_text(json.dumps(machine, indent=2), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "Table_PRODUCT02bk_parallel_reviewer_requirement_readiness.csv")
print("WROTE", TAB / "Table_PRODUCT02bl_parallel_reviewer_readiness_summary.csv")
print("WROTE", TAB / "Table_PRODUCT02bm_current_blockers_for_final_claim.csv")
print("WROTE", TXT / "STAGE1B6K_PARALLEL_reviewer_READINESS_REPORT.md")
