...
echo ============================================================
echo project FULLSPEC EVERYTHING BOOT100
echo Started $(date)
echo ============================================================
...
print()
print("===== MANIFEST =====")
m = pd.read_csv("results/project_fullspec/fullspec_implementation_manifest.csv")
print(m.to_string(index=False))
print("ALL_REQUIRED_FILES_EXIST_AND_NONEMPTY:", bool((m["exists"] & (m["size"] > 0)).all()))

for version in ["raw", "co2corrected"]:
    p = Path(f"results/project_fullspec/fullspec_response_results_{version}.csv")
    print()
    print("====", version.upper(), "====")
...
print()
print("===== TOWER STATUS =====")
print(pd.read_csv("results/project_fullspec/tower_arbiter_status.csv").to_string(index=False))

print()
print("===== TRAIT PROXY =====")
print(pd.read_csv("results/project_fullspec/hierarchical_trait_proxy_results.csv").to_string(index=False))
PY

echo ============================================================
echo project FULLSPEC EVERYTHING BOOT100 FINISHED
echo Finished $(date)
echo ============================================================
