...
OUT = Path("results/stage1b6ao_final_narrowed_project_lock")
TAB = OUT / "tables"
TXT = OUT / "text"
for p in [TAB, TXT]:
...
# Full strict project filter: closure pass + gap <= 0.30 + uWUE valid.
sy["strict_pass"] = (
...
satisfaction = pd.DataFrame([
    ...
    {
        "project_item": "Closure and gap-fill filters",
    ...
    {
        "project_item": "Per-ET-product ranking",
...
satisfaction.to_csv(TAB / "Table_PRODUCT03ci_final_project_satisfaction_matrix_narrowed.csv", index=False)

decision = {
    "generated": now(),
    "stage": "1B.6AO_final_narrowed_project_lock",
    ...
    "c4_status": c4_status,
    "best_c4": best_c4,
    "can_send_to_project": bool(len(strict_sites) >= 1 or len(sens_sites) >= 1),
...
(TAB / "STAGE1B6AO_FINAL_NARROWED_project_LOCK_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")
...
(TXT / "project_READY_RESPONSE_FINAL_NARROWED.md").write_text("\n".join(email), encoding="utf-8")

report = []
report.append("# Stage 1B.6AO final narrowed project lock")
report.append("")
...
(TXT / "STAGE1B6AO_FINAL_NARROWED_project_LOCK_REPORT.md").write_text("\n".join(report), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "STAGE1B6AO_FINAL_NARROWED_project_LOCK_DECISION.json")
...
print("WROTE", TAB / "Table_PRODUCT03ci_final_project_satisfaction_matrix_narrowed.csv")
print("WROTE", TXT / "project_READY_RESPONSE_FINAL_NARROWED.md")
print("WROTE", TXT / "STAGE1B6AO_FINAL_NARROWED_project_LOCK_REPORT.md")
