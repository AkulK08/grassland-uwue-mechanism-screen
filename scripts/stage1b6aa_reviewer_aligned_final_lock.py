...
OUT = Path("results/stage1b6aa_reviewer_aligned_final_lock")
TAB = OUT / "tables"
TXT = OUT / "text"
...
status.to_csv(TAB / "Table_PRODUCT02ex_reviewer_aligned_artifact_status.csv", index=False)
...
response_ready = (
    len(response_lock)
    and str(response_lock["verdict"].iloc[0]) == "FINAL_reviewer_READY_CLAIM_LOCK_COMPLETE"
)
...
reviewer_pathway.to_csv(TAB / "Table_PRODUCT02ey_reviewer_pathway_claims.csv", index=False)
...
verdict = "reviewer_ALIGNED_FINAL_TRAIT_BASED_STORY_READY" if response_ready and trait_ready else "reviewer_ALIGNED_LOCK_NEEDS_REVIEW"
...
decision = pd.DataFrame([{
    ...
    "next_stage": "WRITE_RESULTS_DISCUSSION_AND_reviewer_MEMO",
}])
decision.to_csv(TAB / "Table_PRODUCT02fb_reviewer_aligned_final_decision.csv", index=False)

report = []
report.append("# Stage 1B.6AA reviewer-aligned final trait-based lock")
report.append("")
...
report.append("")
report.append("## reviewer pathway")
report.append("")
report.append("```text")
report.append(reviewer_pathway.to_string(index=False))
report.append("```")
report.append("")
...
(TXT / "STAGE1B6AA_reviewer_ALIGNED_FINAL_TRAIT_LOCK_REPORT.md").write_text("\n".join(report), encoding="utf-8")

machine = {
    "stage": "1B.6AA_reviewer_aligned_final_trait_lock",
    "status": verdict,
    "safe_claim": safe_claim,
    "outputs": {
        "artifact_status": str(TAB / "Table_PRODUCT02ex_reviewer_aligned_artifact_status.csv"),
        "reviewer_pathway": str(TAB / "Table_PRODUCT02ey_reviewer_pathway_claims.csv"),
        ...
        "decision": str(TAB / "Table_PRODUCT02fb_reviewer_aligned_final_decision.csv"),
        "report": str(TXT / "STAGE1B6AA_reviewer_ALIGNED_FINAL_TRAIT_LOCK_REPORT.md"),
    },
}
(TAB / "STAGE1B6AA_reviewer_ALIGNED_FINAL_TRAIT_LOCK_SUMMARY.json").write_text(json.dumps(machine, indent=2), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "Table_PRODUCT02ex_reviewer_aligned_artifact_status.csv")
print("WROTE", TAB / "Table_PRODUCT02ey_reviewer_pathway_claims.csv")
print("WROTE", TAB / "Table_PRODUCT02ez_top_trait_mechanism_effect.csv")
print("WROTE", TAB / "Table_PRODUCT02fa_claims_to_avoid.csv")
print("WROTE", TAB / "Table_PRODUCT02fb_reviewer_aligned_final_decision.csv")
print("WROTE", TXT / "STAGE1B6AA_reviewer_ALIGNED_FINAL_TRAIT_LOCK_REPORT.md")
