...
mkdir -p logs results/project_fullspec data/processed data/external docs

echo "============================================================"
echo "project FULLSPEC SMOKE TEST"
echo "Started: $(date)"
echo "============================================================"
...
ls -lh \
  ...
  scripts/project_build_metric_matrix.py \
  scripts/project_fullspec_analysis.py \
  scripts/project_postprocess_fullspec.py

echo "STEP 2: optional SoilGrids"
if [ ! -s data/external/soilgrids_texture_by_point.csv ]; then
  python scripts/project_fetch_soilgrids_texture.py \
...
python scripts/project_build_metric_matrix.py

echo "STEP 4: run tiny fullspec analysis"
python scripts/project_fullspec_analysis.py \
...
python scripts/project_postprocess_fullspec.py
...
files = [
 "results/project_fullspec/fullspec_response_results_raw.csv",
 "results/project_fullspec/fullspec_response_results_co2corrected.csv",
 "results/project_fullspec/fullspec_vpd_sm_surface_raw.csv",
 "results/project_fullspec/fullspec_vpd_sm_surface_co2corrected.csv",
 "docs/algorithm_dependency_table.csv",
 "docs/trait_causal_dag.md",
 "results/project_fullspec/tower_arbiter_status.csv",
 "results/project_fullspec/hierarchical_trait_proxy_results.csv",
 "results/project_fullspec/fullspec_implementation_manifest.csv",
]
...
p = Path("results/project_fullspec/fullspec_response_results_raw.csv")
if p.exists():
    df = pd.read_csv(p)
...
m = Path("results/project_fullspec/fullspec_implementation_manifest.csv")
if m.exists():
    print(pd.read_csv(m).to_string(index=False))
PY

echo "============================================================"
echo "project FULLSPEC SMOKE TEST FINISHED"
echo "Finished: $(date)"
echo "============================================================"
