...
OUT = Path("results/stage1b6ap_full13_project_lock")
TAB = OUT / "tables"
TXT = OUT / "text"
for p in [TAB, TXT]:
    p.mkdir(parents=True, exist_ok=True)

MANUAL = Path("data/raw/towers/_project_raw_exports/manual_fluxnet")
EXTRACTED = Path("data/raw/towers/_project_raw_exports/extracted")
...
for site in TARGET_SITES:
    ...
    if not has_any_file:
        ...
        status = "COMPUTED_BUT_FAILS_project_QUALITY_FILTER"
...
if len(focal):
    primary_pass = focal.get("primary_controlled_project_pass", pd.Series(False, index=focal.index)).astype(str).str.lower().eq("true").any()
...
satisfaction = pd.DataFrame([
    {
        ...
        "status": "SATISFIED",
        "evidence": screen.iloc[0].get("answer_for_project", "Product-screened table exists."),
    },
    {
    ...
    {
        "project_item": "All 13 raw tower files ingested",
    ...
    {
        "project_item": "Closure and gap-fill computable for all 13",
...
satisfaction.to_csv(TAB / "Table_PRODUCT03cq_full13_project_satisfaction_matrix.csv", index=False)

decision = {
    "generated": now(),
    "stage": "1B.6AP_full13_project_lock",
    ...
    "fully_satisfies_every_project_raw_tower_requirement": bool(full_raw_quality_inputs_13),
    "can_send_FULL_STRICT_without_caveat": bool(full_raw_quality_inputs_13 and has_quality_filtered_ranking),
    "can_send_project_with_explicit_data_availability_caveat": bool(has_quality_filtered_ranking),
    "recommended_action": (
        "SEND_FULL_STRICT_PACKET"
...
(TAB / "STAGE1B6AP_FULL13_project_LOCK_DECISION.json").write_text(json.dumps(decision, indent=2), encoding="utf-8")
...
(TXT / "project_READY_RESPONSE_FULL13_DRAFT.md").write_text("\n".join(email), encoding="utf-8")

report = []
report.append("# Stage 1B.6AP full 13-site project lock")
report.append("")
...
report.append("```")
report.append("")
report.append("## project satisfaction matrix")
report.append("")
report.append("```text")
...
(TXT / "STAGE1B6AP_FULL13_project_LOCK_REPORT.md").write_text("\n".join(report), encoding="utf-8")

print("\n".join(report))
print("")
print("WROTE", TAB / "STAGE1B6AP_FULL13_project_LOCK_DECISION.json")
...
print("WROTE", TAB / "Table_PRODUCT03cq_full13_project_satisfaction_matrix.csv")
print("WROTE", TXT / "project_READY_RESPONSE_FULL13_DRAFT.md")
print("WROTE", TXT / "STAGE1B6AP_FULL13_project_LOCK_REPORT.md")
