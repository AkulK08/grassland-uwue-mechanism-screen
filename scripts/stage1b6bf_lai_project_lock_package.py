...
ROOT = Path.cwd()
OUT = ROOT / "results/stage1b6bf_lai_project_lock_package"
TAB = OUT / "tables"
FIG = OUT / "figures"
TXT = OUT / "text"
        ...
        for gp, et, key in combo_keys:
            ...
            for scenario, mask in scenario_masks.items():
                ...
                rows = compare_model(
                    ...
                    note="Outcome is point-level mean slope_change for this exact GPP×ET product combination. Predictor is growing-season LAI. Controls are full strict controls."
                )
...
for canonical, col, source_type in candidate_predictors:
    ...
    rows = compare_model(
        ...
        note="Discovery-family FDR reconstruction: unique trait/canopy/soil-structure main effects on latent_slope_change under full strict controls."
    )
...
for canonical, col, source_type in candidate_predictors:
    ...
    rows = compare_model(
        ...
        note="Discovery-family FDR reconstruction: trait/canopy/soil-structure × MAT interactions under full strict controls."
    )
...
tower_candidate_paths = [
    ...
    ROOT / "results/stage1b6ay_final_project_audit/tables/Table_PRODUCT03fe_target_tower_landcover_details.csv",
...
analysis_lock = {
    "stage": "stage1b6bf_lai_project_lock_package",
...
- Every main-effect model uses full strict controls.
...
- Each domain reports the LAI main effect and LAI×MAT interaction under the same full strict control set.
...
memo = []
memo.append("Stage1B6BF LAI project lock package")
memo.append("=" * 100)
memo.append("")
...
(TXT / "READ_ME_lai_project_lock_package.txt").write_text("\n".join(memo))

print("\nDONE.")
print(f"Outputs written to: {OUT}")
print("\nPaste this back:")
print(f"cat {TXT / 'READ_ME_lai_project_lock_package.txt'}")
